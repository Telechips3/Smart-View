    ...
    while (true) {
        ...
        if (sem_wait(&q_l->sem_full) == 0) {
            ...
            for (int i = 0; i < item->count; i++) {
                LidarProj p = { Point((int)item->points[i].u, (int)item->points[i].v), 
                                item->points[i].dist, 
                                item->points[i].angle };
                
                // 각도에 따라 전방/후방 분류 (생산자 로직과 매칭)
                // 생산자에서 이미 u, v를 계산해서 넘겨주므로 적절한 view에 할당
                if (abs(p.ang) < 1.57f) ptsF.push_back(p); // 대략 전방 180도
                else ptsR.push_back(p);
            }
        ...
        CameraQueue* q_list[2] = { q_f, q_b };
        for (int cam_id = 0; cam_id < 2; cam_id++) {
            CameraQueue* cur_q = q_list[cam_id];

            if (sem_trywait(&cur_q->sem_full) == 0) {
                pthread_mutex_lock(&cur_q->mutex);
                CameraItem* c_item = &cur_q->buffer[cur_q->head];
                
                Mat& targetView = (cam_id == 1) ? viewF : viewR; // SHM_NAME 정의에 따라 cam_id 매칭
                const vector<LidarProj>& currentPts = (cam_id == 1) ? ptsF : ptsR;
                Scalar color = (cam_id == 1) ? Scalar(0, 255, 0) : Scalar(0, 0, 255);
